...
namespace App.Attributes
{
    ...
    public class UniqueAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            var count = database.Connection.ExecuteScalar<int>($"SELECT COUNT(*) FROM {_table} WHERE {_column} = @value", new { value });

            return (count > (_creating ? 0 : 1)) ? new ValidationResult(FormatErrorMessage(_column)) : ValidationResult.Success;
        }
    }
}